...
export default function RegisterProducerPage(): JSX.Element {
  ...
  const validateCurrentStep = () => {
    ...
    if (currentStep === 1) {
      if (!formData.firstName) newErrors.firstName = "Nombre es requerido";
        ...
        newErrors.firstName = "Nombre debe tener entre 2 y 50 caracteres";
      }
      
      if (!formData.lastName) newErrors.lastName = "Apellido es requerido";
      else if (formData.lastName.length < 2 || formData.lastName.length > 50) {
        newErrors.lastName = "Apellido debe tener entre 2 y 50 caracteres";
      }
      ...
      // Business fields are optional, but validate length if provided
      if (formData.businessName && (formData.businessName.length < 2 || formData.businessName.length > 100)) {
        newErrors.businessName = "Nombre del negocio debe tener entre 2 y 100 caracteres";
      }
      
      if (formData.businessDescription && (formData.businessDescription.length < 10 || formData.businessDescription.length > 500)) {
        newErrors.businessDescription = "Descripción debe tener entre 10 y 500 caracteres";
      }
      
      if (formData.businessAddress && (formData.businessAddress.length < 5 || formData.businessAddress.length > 200)) {
        newErrors.businessAddress = "Dirección debe tener entre 5 y 200 caracteres";
      }
      
      if (formData.businessCity && (formData.businessCity.length < 2 || formData.businessCity.length > 50)) {
        newErrors.businessCity = "Ciudad debe tener entre 2 y 50 caracteres";
      }
      
      if (formData.businessDepartment && (formData.businessDepartment.length < 2 || formData.businessDepartment.length > 50)) {
        newErrors.businessDepartment = "Departamento debe tener entre 2 y 50 caracteres";
      }
  ...
  const validateForm = () => {
    ...
    // Business information validation (optional fields, but validate length if provided)
    if (formData.businessName && (formData.businessName.length < 2 || formData.businessName.length > 100)) {
      newErrors.businessName = "Nombre del negocio debe tener entre 2 y 100 caracteres";
    }
